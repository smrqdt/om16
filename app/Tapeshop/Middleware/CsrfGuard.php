...
class CsrfGuard extends Middleware
{
    ...
    public function check() {
        ...
        // Validate xsrf headers and csrf form tokens
		if($headers->get("X-XSRF-TOKEN")){
			$userToken = $headers->get("X-XSRF-TOKEN");
			if($token !== $userToken){
				$this->app->halt(400, 'Invalid or missing XSRF header.');
			}
		}elseif (in_array($this->app->request()->getMethod(), array('POST', 'PUT', 'DELETE'))) {
		...
		$this->app->setCookie('XSRF-TOKEN', $token, '1 day');
    }
}